...
public class AmazonS3ImageService extends AmazonS3ClientService {
  ...
  public void removeImageFromAmazon(Image image) {
    String fileName = image.getImage_url().substring(image.getImage_url().lastIndexOf("/") + 1);
    getClient().deleteObject(new DeleteObjectRequest(getBucketName(), fileName));
    imageRepository.delete(image);
  }

  private String uploadMultipartFile(MultipartFile multipartFile, UUID id, String folder) {
    String fileUrl;
